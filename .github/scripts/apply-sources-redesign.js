...
content = content.replace(
  ...
  `      '.d365-sources{margin-top:14px;padding-top:12px;border-top:1px solid rgba(0,0,0,0.08);font-size:13px}',
...
content = content.replace(
  `          ? '<span title="' + ref.title + '" style="color:' + config.primaryColor + ';text-decoration:none;font-size:0.75em;vertical-align:super;font-weight:600;cursor:help;">'
          + ref.num + '</span>'
          : '<a href="' + ref.url + '" target="_blank" title="' + ref.title + '" style="color:#0078d4;text-decoration:none;font-size:0.75em;vertical-align:super;">'
          + ref.num + '</a>';`,
  `          ? '<span title="' + ref.title + '" style="display:inline-flex;align-items:center;justify-content:center;width:15px;height:15px;border-radius:4px;background:' + config.primaryColor + '22;color:' + config.primaryColor + ';font-size:9.5px;font-weight:700;vertical-align:super;line-height:1;cursor:help;margin:0 1px;">'
          + ref.num + '</span>'
          : '<a href="' + ref.url + '" target="_blank" title="' + ref.title + '" style="display:inline-flex;align-items:center;justify-content:center;width:15px;height:15px;border-radius:4px;background:#0078d422;color:#0078d4;font-size:9.5px;font-weight:700;vertical-align:super;line-height:1;text-decoration:none;margin:0 1px;">'
          + ref.num + '</a>';`
);
...
if (!content.includes('.d365-sources-header{display:flex')) {
  console.error('ERROR: CSS change 1 did not apply!');
  process.exit(1);
}
if (!content.includes('display:inline-flex;align-items:center;justify-content:center;width:15px')) {
  console.error('ERROR: Pill badge change did not apply!');
  process.exit(1);
}
if (!content.includes('d365-sources-icon')) {
  console.error('ERROR: SVG icon change did not apply!');
  process.exit(1);
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('SUCCESS! All 3 changes applied. File size:', content.length, 'bytes');
